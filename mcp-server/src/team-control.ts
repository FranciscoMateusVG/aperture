import type { ValidateCheckpointSelectors } from "./team-checkpoint-validation.js";
...
// Native lifecycle children own a 170s deadline with a
// bounded cleanup reserve. This parent watchdog is deliberately larger and is
// only a last-resort crash boundary; expiry never means rollback succeeded.
// Checkpoint validation shares this fixed budget/group, never a caller timeout.
...
export type TeamControlRequest =
  ...
  | { action: "retire_seat"; input: StopSeatSelectors & { accept_checkpoint_loss: boolean } }
  | { action: "validate_checkpoint"; input: ValidateCheckpointSelectors }
...
export function teamControlWatchdogMs(action: TeamControlRequest["action"]): number {
  return (action === "retire_seat" || action === "validate_checkpoint" || action === "replace" || action === "bootstrap_seat" || action === "stop_seat" || action === "claude_startup_smoke" || action === "claude_inbox_probe") ? REPLACE_TIMEOUT_MS : action === "archive" || action === "rollback_archive" ? ARCHIVE_TIMEOUT_MS : ORDINARY_TIMEOUT_MS;
}
...
export async function invokeTeamControl(request: TeamControlRequest): Promise<Record<string, unknown>> {
  ...
  return await new Promise((resolve, reject) => {
    const isolated = request.action === "retire_seat" || request.action === "validate_checkpoint" || request.action === "claude_inbox_probe" || request.action === "claude_startup_smoke" || request.action === "stop_seat" || request.action === "bootstrap_seat" || request.action === "replace" || request.action === "archive" || request.action === "rollback_archive";
