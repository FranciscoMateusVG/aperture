...
/** 0 = first start of a never-started seat; 1 = explicit recovery of a first Claude bootstrap that
 *  ended quarantined g1 unobserved. The selector carries no authority: the native child proves it. */
export const bootstrapSeatSchema = z.object({ team: name.max(16), seat: name, expected_generation: z.union([z.literal(0), z.literal(1)]) }).strict();
...
export function bootstrapSelection(value: unknown, input: BootstrapSeatSelectors) {
  ...
  // process_count is persisted incarnation history, not liveness: a recoverable quarantined g1
  // still counts its Gone root. Gone is proved natively, never inferred here.
  const eligible = !!o && (input.expected_generation === 0
    ? o.generation === 0 && o.state === "stale" && o.process_count === 0
    : o.generation === 1 && o.state === "quarantined" && snapshots[0].harness === "claude");
  if (team.state.state !== "active" || !team.capabilities.start || !supported(snapshots[0]) ||
    ...
    throw new Error(input.expected_generation === 0
      ...
      : "E_CONTROL_STALE: seat is not a recoverable quarantined first Claude bootstrap; no automatic retry");
  }
...
export function parseBootstrapStarted(value: unknown, input: BootstrapSeatSelectors, expected: z.infer<typeof tuple>) {
  ...
  // A first start proves generation 1; a recovery of quarantined g1 proves generation 2.
