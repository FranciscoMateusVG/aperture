...
import { claudeStartupSmokeSchema, parseClaudeStartupSmoke } from '../dist/team-claude-smoke.js';
import { invokeTeamControl, teamControlWatchdogMs } from '../dist/team-control.js';

const fixture = name => JSON.parse(readFileSync(new URL(`../../tests/fixtures/team-claude-smoke-${name}.json`, import.meta.url), 'utf8'));
const request = fixture('request');
const input = request.input;

test('shared native wire proves startup only, not tools or available worker', () => {
  assert.deepEqual(claudeStartupSmokeSchema.parse(input), input);
  const result = parseClaudeStartupSmoke(fixture('response'), input).result;
  assert.equal(result.startup, 'verified');
  assert.equal(result.cleanup, 'quarantined');
  assert.equal(result.mcp_readiness, 'not_run');
...
test('strict smoke selectors never accept caller authority, prompts or a second generation', () => {
  for (const delta of [{ actor: 'glados' }, { model: 'sonnet' }, { prompt: 'go' },
    ...
    assert.equal(claudeStartupSmokeSchema.safeParse({ ...input, ...delta }).success, false);
  }
});
...
test('mismatched or overstated diagnostic receipts are UNKNOWN without retry authority', () => {
  for (const delta of [{ team: 'other' }, { seat: 'other' }, { generation: 2 },
    ...
    { cleanup: 'active' }, { mcp_readiness: 'passed' }, { public_enabled: true },
    { thread_id: 'caller' }, { token: 'fixture' }]) {
    const value = fixture('response'); Object.assign(value.result, delta);
    assert.throws(() => parseClaudeStartupSmoke(value, input), /E_CONTROL_UNKNOWN/);
  }
...
function handler(denied, invoke) {
  ...
  const start = source.indexOf('server.tool("team_claude_startup_smoke",');
  ...
  runInNewContext(source.slice(start, end), {
    server: { tool(_name, description, schema, fn) {
      ...
      assert.equal(schema.input, claudeStartupSmokeSchema); callback = fn;
    } },
    gladosControlDenied: () => denied, claudeStartupSmokeSchema, parseClaudeStartupSmoke,
...
test('native diagnostic transport uses stdin, isolated process group and fixed 180s watchdog', async () => {
  assert.equal(teamControlWatchdogMs('claude_startup_smoke'), 180_000);
