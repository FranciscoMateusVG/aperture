...
- **C-7 Closure.** A work-bearing task closes when its PR is opened, not merged; if the bead's own acceptance names a QA gate or reviewer as the closing condition, that acceptance governs and the task stays open until that explicit verdict is recorded.
...
- **C-9 Delegate first, review always.** On any non-trivial task decompose first and fan parallelizable work out to subagents in proportion to the task — tiny tasks are done locally, dispatch stays bounded to what was authorized, and you never self-authorize new tooling, hardening or extra investigation tracks — keeping for yourself only design decisions, the single craft centerpiece and the review; every subagent diff is read before sign-off, and no test-pass claim is trusted on a file someone else edited concurrently.
...
- **C-11 Verify against reality.** Claims about the canonical branch, deploys, PRs, sessions or another agent's work are checked at the canonical artifact (the repo's canonical remote branch — `origin/main`, `master` or `staging` as that repo defines it — the live service, `gh pr view`, the transcript) before they are acted on or repeated — including your own earlier claims.
...
- **C-13 Handoffs name the reviewer.** Work is not done until the named reviewer (Izzy for implementation) has been told what changed, which files were touched and what to test — and notifying the reviewer is not sign-off: where the bead's acceptance requires explicit QA approval, closure waits for that verdict; non-PR work such as specs closes on its own acceptance, naming the implementer and the PR-owning bead.

## B. Module index — load on demand, not at boot

Loaded text stays in context for the session: load one module when its trigger applies, not several "just in case". §A already covers routine operation — claiming, updating, acknowledging, closing, reporting — so the ordinary first task should need no module at all; go to a full document only for a procedure or edge case §A does not state. Paths are repo-relative to `.claude/skills/`.

| load when… | module (path) | size | what it adds beyond §A |
|---|---|---|---|
| a bead case §A does not cover — filing (GLaDOS only), epics and parent wiring, project-label taxonomy, artifact-type or auto-closed-PR edge cases — NOT an ordinary claim / progress note / artifact / close, which C-5, C-7 and C-8 already cover | `beads/SKILL.md` | 20 KB | full lifecycle detail, epic wiring, label taxonomy, tool-argument escaping specifics |
| an infra deploy handoff to Peppy, a per-layer "feature live" verify chain, or a status-report format question — NOT an ordinary send_message / mark_as_read / doorbell, which C-2 and C-3 already cover | `communicate/SKILL.md` | 11 KB | §6 deploy-handoff template, §7.2 per-layer verify chains, §4 report format |
| you need to know who does what, or who to route a question to | `team/SKILL.md` | 5 KB | roster and lanes |
| cutting or cleaning up a worktree; stacked PRs (§7); squash-merge rebases; shared-worktree collisions (§8) | `worktree-discipline/SKILL.md` | 7 KB | setup/cleanup commands, `rebase --onto` recovery, auto-close recovery, hygiene audit |
| deciding subagent fan-out vs Agent Teams; serial-vs-parallel dispatch framing; context budget mechanics | `specialist-delegation/SKILL.md` | 15 KB | when to delegate vs stay hands-on, worked examples, §7 context-budget/compact mechanics, §8 parallel tracks |
