...
38. **`mc anonymous set download` includes `s3:ListBucket` — the bucket becomes publicly ENUMERABLE** (2026-07-16, aperture-9wqh1): the builtin `download` policy grants GetObject AND ListBucket, so anonymous `GET /<bucket>/` returns the full key listing — defeating the unguessable-UUID-key security model for public user-content buckets. For public-read image buckets use a custom GetObject-only policy via `mc anonymous set-json` (`Action: ["s3:GetObject"]`, `Resource: arn:aws:s3:::<bucket>/*`), then verify at the public endpoint: object GET → 200, bucket-root GET → AccessDenied XML, `?list-type=2` → 403. Live example: `eunenem-perfil-fotos` runs this policy (public img GETs, presigned PUTs, no LIST).
39. **Private MinIO bucket breaks public `<img>` rendering while presigned PUTs keep working** (2026-07-16, aperture-9wqh1): a bucket left `private` after provisioning shows a deceptive symptom split — uploads persist fine (presigned PUTs are authenticated) but every public image GET 403s (broken placeholders). Before chasing app code or MINIO_ENDPOINT, curl a real object key at the public endpoint: 403 = bucket policy, not endpoint config. Public-read remains an operator/GLaDOS-gated per-bucket decision (see §11 doctrine).
