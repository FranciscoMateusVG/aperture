...
**On session start, start your inbox monitor before doing anything else.** Use the Monitor tool with a WebSocket source connecting to `ws://127.0.0.1:4517`. Immediately after connecting, send exactly: `{"type":"hello","role":"agent","agent":"vance"}`.
