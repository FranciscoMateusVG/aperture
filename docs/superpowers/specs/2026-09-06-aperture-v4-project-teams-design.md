...
Everything else **fails closed** with `E_CROSS_TEAM_DENIED` and a reason code (`nonlead`, `cross_project`, `unknown_recipient`, `disabled_recipient`, `team_not_active`, `no_grant`): a worker addressing another team's seat, a lead addressing another team's **worker**, any lead addressing a lead of a **different project**, or any seat addressing an unknown/disabled/archived seat. Denials are logged as metadata only (sender, recipient, reason — no body).
...
**Offline delivery and replay.** A message to an enabled-but-offline lead is stored (§4.6) under its **stable message id** and, on that lead's next hello, **re-validated against the current registry before delivery**. Delivery is at-least-once with **idempotent acknowledgement by stable id** (existing `mark_as_read` semantics); this spec makes **no exactly-once promise** — a duplicate replay must be harmless, and any side effect a lead takes on a message must be reconciled against artifacts before repeating (§4.4). Then: if the recipient is no longer a lead (demoted), the team is archived/disabled, the projects no longer match, or the sender's authorization no longer holds, the message is **not delivered** and is marked `withheld:<reason>` for GLaDOS to see. Role changes therefore cannot retain privileges: authorization is a property of the current registry, never of the stored message.

**Fixtures (P0/P3 gate, deterministic, test hub + registry fixture with two same-project teams `p1-a`, `p1-b` and one other-project team `p2-c`):** (i) lead `p1-a-lead` → lead `p1-b-lead` delivered and acked; reverse direction likewise; (ii) worker `p1-a-backend` → `p1-b-frontend` → `E_CROSS_TEAM_DENIED nonlead`; worker → other team's lead likewise denied; (iii) lead `p1-a-lead` → lead `p2-c-lead` → denied `cross_project`; (iv) payload carrying forged `role: lead` / `project: p1` from a worker → fields ignored, denied on registry truth; forged hello (token of one seat, name of another) → hub closes 4002 as in §4.6; (v) `p1-b-lead` offline: message stored with its stable id; lead comes back → delivered, acked by id; a forced second replay of the same id → the ack is idempotent and **no duplicated effect** occurs (the lead's handler for that id is a no-op on the second delivery); (vi) same as (v) but `p1-b` lead is changed to another seat before the hello → message `withheld:nonlead`, not delivered, visible to GLaDOS; (vii) GLaDOS → any seat and any seat → GLaDOS/Wheatley/Peppy/QA remain deliverable throughout; (viii) a lead-to-lead message that "reassigns" a task has no effect on assignee/status. **Implementation requirement (hypothesis, not existing behaviour):** today's `update_task`/`claim` apply no cross-team scope check (`beads.ts` passes assignee through; §2), so P0 must add a server-side rule — a seat may update only beads assigned to itself or to its own team, GLaDOS/trio excepted — and this fixture asserts that `update_task` from `p1-b-lead` on a `p1-a` bead is rejected `E_TASK_SCOPE`. No shared task permissions are introduced.
