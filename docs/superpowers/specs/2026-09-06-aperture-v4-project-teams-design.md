...
| **Worker seat** | scoped task execution and evidence on its own beads; checkpoints; reporting through its lead | ungranted direct cross-team messaging (ordinary coordination goes via its lead; trio paths and recorded scoped grants per §4.10 remain); queue discovery; self-claim of unassigned work |
...
| GLaDOS (PR #66 review) + operator | lead-of-leads model; no unevidenced task-scope claim; no blanket QA path; no exactly-once promise | §4.12 responsibility table + escalation path; §4.10 (viii) = no task ACL, BEADS access Constitution-governed, `send_message` causes no mutation; QA path = explicit scoped grant in registry (`no_grant` reason); replay = at-least-once with idempotent stable-id ack, fixture (v) asserts no duplicated effect; §4.11 lead-specific guidance |
