...
**Visual contract (P2):** reference screenshots at 1280×800 and 1024×768 for library, wizard, grouped view, Replace dialog (each state), Archive checklist; assertion = invariants (no clipping/overflow, all CTAs visible, text not truncated) plus a ≤ 1 % pixel-delta budget against the reference; keyboard: tab order matches visual order, dialogs trap focus and return it on close, Escape = Cancel; labels associated; WCAG AA contrast; targets ≥ 44×44 px; **every CTA clicked once** in the journey.
