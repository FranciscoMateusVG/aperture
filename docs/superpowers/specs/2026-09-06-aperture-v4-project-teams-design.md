...
3. **Explicitly authorized paths, unchanged:** any seat ↔ the coordination trio (GLaDOS, Wheatley, Peppy) and ↔ the QA seat designated for the project; GLaDOS ↔ anything (oversight is not reduced by this section); `operator` doorbell.
Everything else **fails closed** with `E_CROSS_TEAM_DENIED` and a reason code (`nonlead`, `cross_project`, `unknown_recipient`, `disabled_recipient`, `team_not_active`): a worker addressing another team's seat, a lead addressing another team's **worker**, any lead addressing a lead of a **different project**, or any seat addressing an unknown/disabled/archived seat. Denials are logged as metadata only (sender, recipient, reason — no body).

**What the channel does and does not grant.** Lead-to-lead messages are coordination: sharing interfaces, sequencing, blockers, review requests. They confer **no authority**: the receiving lead owns delegation inside their own team; a message cannot reassign or cancel another team's task, add scope, or change any seat's model/budget (those follow §4.6/§4.9 and the operator's approval rules). No shared inbox and no cross-team task read/claim access is introduced — `query_tasks`/`update_task` keep their existing per-agent scope.

**Offline delivery and replay.** A message to an enabled-but-offline lead is stored (§4.6) and, on that lead's next hello, **re-validated against the current registry before delivery**: if the recipient is no longer a lead (demoted), the team is archived/disabled, the projects no longer match, or the sender's authorization no longer holds, the message is **not delivered** and is marked `withheld:<reason>` for GLaDOS to see. Role changes therefore cannot retain privileges: authorization is a property of the current registry, never of the stored message.

**Fixtures (P0/P3 gate, deterministic, test hub + registry fixture with two same-project teams `p1-a`, `p1-b` and one other-project team `p2-c`):** (i) lead `p1-a-lead` → lead `p1-b-lead` delivered and acked; reverse direction likewise; (ii) worker `p1-a-backend` → `p1-b-frontend` → `E_CROSS_TEAM_DENIED nonlead`; worker → other team's lead likewise denied; (iii) lead `p1-a-lead` → lead `p2-c-lead` → denied `cross_project`; (iv) payload carrying forged `role: lead` / `project: p1` from a worker → fields ignored, denied on registry truth; forged hello (token of one seat, name of another) → hub closes 4002 as in §4.6; (v) `p1-b-lead` offline: message stored; lead comes back → delivered exactly once; (vi) same as (v) but `p1-b` lead is changed to another seat before the hello → message `withheld:nonlead`, not delivered, visible to GLaDOS; (vii) GLaDOS → any seat and any seat → GLaDOS/Wheatley/Peppy/QA remain deliverable throughout; (viii) a lead-to-lead message that "reassigns" a task has no effect on assignee/status (authority test: `update_task` from the receiving lead on the other team's bead is rejected by existing scope rules).
