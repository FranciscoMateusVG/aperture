...
1. **Mission delegation.** The lead receives the mission from GLaDOS and decomposes it into seat beads; every task bead keeps its **actual seat as assignee** (`t1-backend`, not the lead) so ownership and BEADS history stay truthful.
2. **Consolidated reporting.** Workers report progress and blockers **through the lead**; the lead sends GLaDOS one consolidated progress/blocker report per milestone (and immediately on any blocker), in the standard shape: what changed, files/PRs, blockers with owner, next step. Workers still write their own bead notes/artifacts; the lead's report links to them rather than restating them. GLaDOS may still read any seat bead directly.
3. **Pre-archive reconciliation.** Before requesting archive the lead walks **every** mission bead, seat bead and required review and records a **reconciliation record** on the epic bead, one line per item, each with exactly one disposition:
   ...
   - **cancelled** — only by an explicit, authorized disposition (operator or GLaDOS decision recorded on the bead with reason); the lead may propose, never self-authorize;
   - **deferred (transferred)** — an explicit, **accepted** handoff to a **named owner and task outside the closing team**: the bead's assignee changes to the new owner, a `transferred-from:<seat>` note plus the original history stays on the bead, and the receiving owner acknowledges the transfer in writing (BEADS message referenced in the record). Until acceptance the item counts as unresolved.
...
4. **Archive request.** The lead requests archive only when the reconciliation record shows zero unresolved items; the archive checklist (§3.5/§4.7) displays the record and **blocks** on: any item without a disposition, any `deferred` without recorded acceptance, any required review missing, any bead still assigned to a seat of the closing team.

**Fixtures (P4 gate):** (a) one seat bead left open with unmet acceptance → archive blocked with that bead named; (b) a bead marked completed without evidence → reconciliation validator flags `completed-without-evidence`, archive blocked; (c) a proposed cancellation without an operator/GLaDOS disposition note → blocked; (d) a transfer to a named external owner **without** their acknowledgement → blocked; the same transfer **with** acknowledgement → passes, bead assignee changed, `transferred-from` note and full prior history intact, original team's history untouched; (e) lead report fixture: two workers post blockers to the lead → GLaDOS receives exactly one consolidated report naming both blockers and their owners within the milestone window, and each worker's own bead notes still carry the original detail.
