...
| standing-rule retention — named obligation coverage (see §7a) | (a) all **14 enumerated DECISION rows** (DECISION-1, DECISION-1b, DECISION-2, DECISION-3, DECISION-4, DECISION-5, DECISION-6, DECISION-7, DECISION-8, DECISION-9, DECISION-10, DECISION-11, DECISION-12, DECISION-13) present verbatim in resident `orchestrator-core/SKILL.md §0`, checked by id; (b) all **13 designated standing memory statements** (sidecar `standing:true`, listed by key in the gate output) present in both `boot` and `precompact` renders. Two disjoint sets; the gate prints every id/key it verified, never a bare count. Historical measurement (2026-09-06, before DECISION-13): 13/13 and 13/13 boot + 13/13 precompact; not a current runtime receipt |
...
**What the 25 was.** v1 of this spec (`feaf0c2`) wrote "25 named constitutional rules" in §7 *before* `DECISIONS.md` existed, while the same v1 §1 already sized the reconciled rule set at **13 overlap clusters**. At the original 2026-09-06 audit, the reviewed branch enumerated 13 decisions and did not yet contain `DECISION-13`. Operator-approved `aperture-tgx8c` added DECISION-13 on 2026-09-07; the old no-13 assertion is historical, not a claim about the current tree. The 25 was an **unenumerated draft estimate** carried into v1.1, not a list anything was measured against — so nothing that was ever enumerated has been dropped. This section replaces the estimate with the enumerated set.

**The current enumerated set = the 14 DECISION rows** (`DECISION-1, 1b, 2 … 13` in `orchestrator-core/DECISIONS.md`). The original 13 were GLaDOS-reviewed before merge per §1; DECISION-13 adds the operator-approved 2026-09-07 bounded-ownership, proportional-QA and native-delivery rule. The table below records coverage of the original source-skill reconciliation (derived from those rows' *Supersedes* / *Source* columns):
