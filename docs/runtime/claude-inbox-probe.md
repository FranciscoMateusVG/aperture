...
The receipt reports startup `verified`, kickoff `sent`, owner `active`, MCP
readiness `pending`, reasoning `not_observed`, public enabled `false`. It does
...
The diagnostic remains Active for the real inbox check. Errors after startup
use the same reservation's bounded cleanup; cleanup uncertainty takes priority
over any earlier observation. No automatic replacement or second attempt.
...
4. A BEADS test message is fetched, answered, and marked read by the same seat.
...
5. Preserve/record the checkpoint and use the authorized stop/archive path; do
   not reset an owner or invent a checkpoint to make the UI clean.
