...
export function renderTeamGroup(team: TeamView, agents: readonly AgentDef[] = []): string {
  ...
  return `<section class="v4-card v4-team"><div class="v4-toolbar v4-team__head"><div class="v4-team__title"><h2 title="${e(s.team)}">${e(s.team)}</h2><p class="v4-meta">${count}</p></div><div class="v4-team__state"><span class="v4-badge v4-status">${e(state.state)}</span></div></div>
    ...
    ${active ? `<ul class="v4-seats">${team.seats.map(({ configured, observed_owner: owner }) => `<li class="v4-seat" data-seat-row="${e(configured.name)}"><div class="v4-seat__row">${badge(seatStatus(owner, turnOf(configured.name)))}<span class="v4-seat__name" title="${e(configured.name)}">${e(configured.name)}${configured.name === s.lead ? " · LEAD" : ""}</span><span class="v4-seat__role">${e(configured.role)}</span>${openControl(configured.name)}</div>
    ...
    <details class="v4-details" data-details="team:${e(s.team)}"><summary>Mission, acceptance and team actions</summary><p class="v4-meta">${technical}</p><p>${e(s.mission)}</p><p class="v4-meta">Acceptance: ${e(s.acceptance)}</p><div class="v4-actions"><button class="v4-button" data-action="archive" data-team="${e(s.team)}">Archive checklist…</button></div></details>
