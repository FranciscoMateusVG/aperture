...
export function renderTeamGroup(team: TeamView, agents: readonly AgentDef[] = []): string {
  ...
  const openControl = (name: string) => canOpenSeat(team, name)
    ? `<button class="v4-button v4-button--small" data-action="open-seat" data-team="${e(team.snapshot.team)}" data-seat="${e(name)}">Abrir</button>`
    : `<button class="v4-button v4-button--small" data-action="open-seat" data-team="${e(team.snapshot.team)}" data-seat="${e(name)}" disabled title="Agente não disponível para abrir">Abrir</button>`;
  const terminal = (name: string) => canOpenSeat(team, name) ? "Disponível — abre a sessão existente, nunca inicia um agente" : "Agente não disponível para abrir";
  ...
  const count = active ? `${team.seats.length} agentes · ${working} trabalhando · ${waiting} aguardando` : `${s.seats.length} agentes`;
  const shortName = (name: string) => name.startsWith(`${s.team}-`) ? name.slice(s.team.length + 1) : name;
  // Technical identity stays available, but only inside the team details: the visible header is team name + count + lifecycle.
  const technical = `repository: ${e(s.repo)} (immutable) · ${e(s.project)} · lead: ${e(s.lead)} · generation: g${state.generation} · ${state.epic_id ? `epic: ${e(state.epic_id)}` : "epic: pending"}`;
  return `<section class="v4-card v4-team"><div class="v4-toolbar v4-team__head"><div class="v4-team__title"><h2 title="${e(s.team)}">${e(s.team)}</h2><p class="v4-meta">${count}</p></div><div class="v4-team__state"><span class="v4-badge v4-status">${e(state.state)}</span></div></div>
    ...
    ${active ? `<ul class="v4-seats">${team.seats.map(({ configured, observed_owner: owner }) => `<li class="v4-seat" data-seat-row="${e(configured.name)}"><div class="v4-seat__row"><span class="v4-seat__identity"><span class="v4-seat__name" title="${e(configured.name)}">${e(shortName(configured.name))}${configured.name === s.lead ? " · líder" : ""}</span>${badge(seatStatus(owner, turnOf(configured.name)))}</span>${openControl(configured.name)}</div>
      <details class="v4-details v4-seat__details" data-details="seat:${e(s.team)}:${e(configured.name)}"><summary aria-label="Detalhes de ${e(configured.name)}" title="Detalhes de ${e(configured.name)}"><span aria-hidden="true">⌄</span></summary><dl class="v4-seat-facts"><div><dt>Agente / função</dt><dd>${e(configured.name)} · ${e(configured.role)}</dd></div><div><dt>Snapshot · immutable configuration</dt><dd>${e(tupleLabel(configured))}</dd></div><div><dt>Requested · current incarnation</dt><dd>${owner ? e(tupleLabel(owner.configured)) : "Unknown — no owner observation"}</dd></div><div><dt>Observed model</dt><dd>${owner?.actual ? e(tupleLabel(owner.actual)) : "Unknown — not observed"}</dd></div><div><dt>Owner / generation</dt><dd>${owner ? `${e(owner.state)} · g${owner.generation}` : "Unknown — no owner observation"}</dd></div><div><dt>Terminal</dt><dd>${terminal(configured.name)}</dd></div><div><dt>Checkpoint / context</dt><dd>Not available from this backend</dd></div><div><dt>Current turn</dt><dd>${e(turn(configured.name))}</dd></div><div><dt>Thread binding</dt><dd>${owner ? owner.thread_bound ? "Bound (reported)" : "Not bound" : "Unknown"}</dd></div></dl>
      <div class="v4-actions"><button class="v4-button" disabled title="Checkpoint command is not integrated">Checkpoint unavailable</button><button class="v4-button" data-action="replace" data-team="${e(s.team)}" data-seat="${e(configured.name)}">Replace worker…</button><button class="v4-button" disabled title="Team stop requires the verified lifecycle protocol">Stop unavailable</button></div></details></li>`).join("")}</ul>` : ""}
    <details class="v4-details" data-details="team:${e(s.team)}"><summary>Detalhes do time</summary><p class="v4-meta">${technical}</p><p class="v4-meta">Inicialização coordenada por GLaDOS; sem início manual pelo launcher.</p><p>${e(s.mission)}</p><p class="v4-meta">Acceptance: ${e(s.acceptance)}</p><div class="v4-actions"><button class="v4-button" data-action="archive" data-team="${e(s.team)}">Archive checklist…</button></div></details>
