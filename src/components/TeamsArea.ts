...
export function renderTeamGroup(team: TeamView): string {
  ...
  return `<section class="v4-card"><div class="v4-toolbar"><div><h2>${e(s.team)}</h2><p class="v4-meta">${e(s.project)} · lead: ${e(s.lead)} · ${state.epic_id ? `epic: ${e(state.epic_id)}` : "epic: pending"}</p></div><span class="v4-badge v4-status">${e(state.state)} · g${state.generation}</span></div><p>${e(s.mission)}</p><p class="v4-meta">Acceptance: ${e(s.acceptance)}</p>
    ...
    ${active ? `<div class="v4-stack">${team.seats.map(({ configured, observed_owner: owner }) => `<article class="v4-card"><h3>${e(configured.name)}${configured.name === s.lead ? " · LEAD" : ""}</h3>
      <dl class="v4-seat-facts"><div><dt>Configured</dt><dd>${e(tupleLabel(configured))}</dd></div><div><dt>Observed model</dt><dd>${owner?.actual ? e(tupleLabel(owner.actual)) : "Unknown — not observed"}</dd></div><div><dt>Owner / generation</dt><dd>${owner ? `${e(owner.state)} · g${owner.generation}` : "Unknown — no owner observation"}</dd></div><div><dt>Checkpoint / context</dt><dd>Not available from this backend</dd></div><div><dt>Current turn</dt><dd>Unknown — no turn observation</dd></div><div><dt>Thread binding</dt><dd>${owner ? owner.thread_bound ? "Bound (reported)" : "Not bound" : "Unknown"}</dd></div></dl>
...
export function createTeamsArea(container: HTMLElement, legacyRoster: HTMLElement, options: TeamsAreaOptions) {
  ...
  async function refresh() {
    ...
    try {
      ...
      teamsEl.innerHTML = teams.length ? teams.map(renderTeamGroup).join("") : '<p class="v4-notice">No teams registered. Your standing roster is unchanged.</p>';
